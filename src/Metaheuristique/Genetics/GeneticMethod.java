...
public class GeneticMethod {
    ...
    /*public static ArrayList<Solution> generateAllCrossover(ArrayList<Solution> populationk)
    {
        Solution solutionGeneratedByCrossoverRBX;
        Solution solutionGeneratedByCrossoverSBX;


        ArrayList<Solution> popGeneratedByCrossover = new ArrayList<>();
        for (int sol1 = 0; sol1 < populationk.size()-1; sol1++) {
            for (int sol2 = sol1+1;sol2<populationk.size();sol2++)
            {
                Solution solution1 = populationk.get(sol1).clone();
                Solution solution2 = populationk.get(sol2).clone();

                for (int road1 = 0; road1<solution1.getRoads().size();road1++)
                {
                    for(int road2=0;road2<solution2.getRoads().size();road2++)
                    {
                        // Solution generated by RBX
                        solutionGeneratedByCrossoverRBX = Crossover.crossOverRBX(solution1,solution2, road1, road2);

                        // Solution generated by SBX
                        for (int indexBeginningRoad1=1; indexBeginningRoad1<solution1.getRoads().get(road1).getDestinations().size()-2; indexBeginningRoad1++)
                        {
                            for(int indexEndRoad2=1;indexEndRoad2<solution2.getRoads().get(road2).getDestinations().size()-2;indexEndRoad2++)
                            {
                                solutionGeneratedByCrossoverSBX = Crossover.crossOverSBX(solution1,solution2, road1, road2, indexBeginningRoad1, indexEndRoad2);

                                // Add the solution generated by SBX to the population if not null
                                if(solutionGeneratedByCrossoverSBX != null)
                                {
                                    popGeneratedByCrossover.add(solutionGeneratedByCrossoverSBX);
                                }
                            }
                        }

                        // Add the solution generated by RBX to the population if not null
                        if(solutionGeneratedByCrossoverRBX != null)
                        {
                            popGeneratedByCrossover.add(solutionGeneratedByCrossoverRBX);
                        }
        ...
        return popGeneratedByCrossover;
    }*/
        ...
        for(int k=0; k<maxGeneration; k++)
        {
            ...
            while (i != numberSolutionToGenerate)
            {
                ...
                if (new Random().nextFloat() < mutationRate || populationk.size() <=1)
                {
                    ...
                    //Trouver la meilleure solution et l'ajouter
                ...
                else
                {
                    ...
                    while(solutionGeneratedByCrossover == null && counter < 2*populationSize)
                    {
                        ...
                        {
                            int indexRoad1 = new Random().nextInt(solution1.getRoads().size());
                            int indexRoad2 = new Random().nextInt(solution2.getRoads().size());
                            //System.out.println("Crossover RBX selectionne");
                            solutionGeneratedByCrossover = Crossover.crossOverRBX(solution1,solution2, indexRoad1, indexRoad2);
                        ...
                        else
                        {
                            ...
                            int indexRoad1 = new Random().nextInt(solution1.getRoads().size());
                            int indexRoad2 = new Random().nextInt(solution2.getRoads().size());
                            solutionGeneratedByCrossover = Crossover.crossOverSBX(solution1,solution2, indexRoad1, indexRoad2);
                        }
                        counter++;
